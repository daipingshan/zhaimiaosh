...
class LoginController extends CommonController {
    ...
    public function __construct() {
        ...
        if (in_array(ACTION_NAME, $auth_arr) && session('partner_id') > 0) {
    ...
    public function doLogin() {
        ...
        $partner->where(array('id' => $user['id']))->save(array('last_time' => time(), 'last_ip' => get_client_ip()));
        session('partner_id', $user['id']);
        session('partner_name', $user['partner_name']);
        $this->success('登录成功！');
    }
