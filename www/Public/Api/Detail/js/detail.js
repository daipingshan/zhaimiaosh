/**
 * Created by Leester on 2016/12/30.
 */
...
$(function () {
    ...
    //打开蒙版
    $(".detail-coupon-buy").on("click", function () {
        if (isWeiXin()) {
            openTips();
            return false;
        }
...
function loadData() {
    ...
    loading = false;
    $.ajax({
        url: "http://hws.m.taobao.com/cache/mtop.wdetail.getItemDescx/4.1/?data=%7B%22item_num_id%22%3A%22" + $("#wxGid").val() + "%22%7D&type=jsonp",
        dataType: "jsonp",
        timeout: 3000,
        success: function (data) {
            ...
            for (var i = 0; i < imgLen; i++) {
                imgArr.push('<img class="lazy" src="/Public/Api/Detail/images/default-pic2.jpg" data-img="' + imgData[i] + '" alt="">');
            }
            $("#content").html(imgArr);
            $(loadEle).hide();
    ...
    if (isShow == 1) {
        ...
        $(".copy-tao-words .copy-tao-words-btn").on("click", function () {
            ...
            clipboard.on('error', function (e) {
                e.trigger.innerHTML = "复制失败，请手动选择复制";
                e.trigger.style.backgroundColor = "#999";
                e.trigger.style.borderColor = "#999";
                setTimeout(function () {
                    $(".copy-tao-words").hide();
                    $(".media-tkl").show();
                    $('.self-copy-area').show();
